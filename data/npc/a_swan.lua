...
local function creatureSayCallback(npc, creature, type, message)
	...
	if MsgContains(message, 'mission') then
		if player:getStorageValue(Storage.Quest.ThreatenedDreams.Mission01[1]) == 11 then
			npcHandler:say({
		...
		elseif player:getStorageValue(Storage.Quest.ThreatenedDreams.Mission01[1]) == 14 then
			if player:getItemCount(25244) >= 5 then
				player:removeItem(25244, 5)
				player:setStorageValue(Storage.Quest.ThreatenedDreams.Mission01[1], 15)
				npcHandler:say({
	...
	elseif MsgContains(message, "yes") then
		if npcHandler:getTopic(playerId) == 1 then
			...
			player:setStorageValue(Storage.Quest.ThreatenedDreams.Mission01[1], 12)
			npcHandler:setTopic(playerId, 0)
		end
	end
	return true
