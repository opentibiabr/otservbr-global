...
local function creatureSayCallback(npc, creature, type, message)
	...
	------------------------ FINISH MISSION 01 ------------------------
		elseif(getPlayerStorageValue(creature, Storage.RottinWoodAndMaried.Mission03) == 2) then
			npcHandler:say("Of course, of course, there is indeed something you can help us with. Remember that we also have some tasks for you. So, are you ready for another quest to help the men of the forest?", npc, creature)
			npcHandler:say("There is a problem with one of our deer stands. Right. Well, there are two problems, our deer stands - and some of the walls of the buildings in the camp are broken. ...", npc, creature)
			npcHandler:say("You know, the guys built all of that themselves. Sure, at first it didn't quite work out as we planned and in the end we had to tear down half the forest - but - it was worth it. ...", npc, creature)
			npcHandler:say("Still, most of the camp is kind of... broken now. And someone with a good hammer and a steady hand needs to fix that. Or I am afraid we will have to freeze... during the cold evenings... well you know, hard times. ...", npc, creature)
	...
	------------------------ FINISH MISSION 02 ------------------------
		elseif(getPlayerStorageValue(creature, Storage.RottinWoodAndMaried.Mission03) == 4) then
			npcHandler:say("Oh my good friend, good to see you! Today you will help us with a very important task. Very important indeed. ...", npc, creature)
			npcHandler:say("You know, a large group of merchants is travelling from Thais to Venore and they are crossing the forest to shorten their way - can you believe it? ...", npc, creature)
	...
	elseif(MsgContains(message, "yes")) then
		...
		elseif(npcHandler:getTopic(playerId) == 2) then
			npcHandler:say("Fine, now hurry into the woods and find some rabbits. Find them and kill them that is. Use a sharp, thin knife to get the rabbit foot and be careful not to destroy it - no one would buy it in a bad condition after all. ...", npc, creature)
			npcHandler:say("Once you gathered some rabbit feet, put one of our - handcrafted by the guys - ribbons on each of them. That will make two lucky charms per rabbit. ...", npc, creature)
			npcHandler:say("We will need at least seven. And seven is a lucky number, don't you think? Heh. ...", npc, creature)
		...
		elseif(npcHandler:getTopic(playerId) == 5) then
			npcHandler:say("Mmmmh, I have to say - good workmanship. No doubt. Yes, the person who made that tool you used to fix all this was a pure professional. Something I can't say about your work, though. ...", npc, creature)
		...
		elseif(npcHandler:getTopic(playerId) == 6) then
			npcHandler:say("Right, now before the merchants enter the woods, you will install several traps - nothing too dangerous, just nets and ropes. ...", npc, creature)
			npcHandler:say("When they enter said area which is located close to our hidden camp, they will be trapped and you can... lighten their heavy burden of valuable goods. ...", npc, creature)
			npcHandler:say("There is but a tiny little catch - we cannot help you laying these traps. You know, some of the guys became sick recently, we talk about several broken legs here, one almost lost his arm ...", npc, creature)
			npcHandler:say("So... it's entirely up to you, will you do this or not?", npc, creature)
			npcHandler:setTopic(playerId, 7)
		elseif(npcHandler:getTopic(playerId) == 7) then
			npcHandler:say("Alright very good. Now the only thing you need to do is taking these traps and moving out to the area I have marked on your map where you need to place them. ...", npc, creature)
			npcHandler:say("Once you did that go to the large high seat near the camp and watch for the travelling merchants. ...", npc, creature)
			npcHandler:say("Once all of them are trapped in the nets, you can go down and gather anything of value you can find. But only take what we... you really need - around 100 gold should be enough for any man to take. ...", npc, creature)
			npcHandler:say("We have manners after all, haven't we. ...", npc, creature)
			npcHandler:say("My men will take their share of course and... help you relieve the merchants of any valuables. ...", npc, creature)
			npcHandler:say("You're done if you get at least... let's say 5 of these fools. Return to me and you will be rewarded. ...", npc, creature)
